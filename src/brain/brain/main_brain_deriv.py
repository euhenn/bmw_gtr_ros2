...
from mpc_controller_deriv import MPC_KinematicBicycle
...
class CarControllerNode(Node):
    ...
    def get_current_state(self):
        ...
        delta = np.deg2rad(float(self.car.curr_steer))
        ...
        while rclpy.ok():
            ...
            try:
                ...
                if (False):
                    ...
                    L = self.mpc.L
                    beta = np.arctan(self.mpc.lr * np.tan(delta) / self.mpc.L)
                    ...
                    delta_pred = delta + ddelta_prev * delay_sec
                    if v_pred < 0.0:
                        v_pred = 0.0
                    elif v < 0.05 and a_prev > 0.0:
                        v_pred = max(v_pred, 0.05)
                    state_ocp, idx = self.mpc.get_state(x_pred, y_pred, yaw_pred, v_pred, delta_pred)
                else:
                    # === 2. Transform to spatial coordinates ===#
                    state_ocp, idx = self.mpc.get_state(x, y, yaw, v, delta)
                ...
                v_cmd = max(0.1, v + (a_cmd * loop_duration))
                delta_cmd = delta + ddelta_cmd * loop_duration
                # Optionally clamp steering to realistic range (e.g., ±30°)
                max_steer = np.deg2rad(30)
                delta_cmd = np.clip(delta_cmd, -max_steer, max_steer)

                self.apply_control(v_cmd, delta_cmd)

                a_prev, ddelta_prev = a_cmd, ddelta_cmd

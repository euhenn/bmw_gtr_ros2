...
class CarControllerNode(Node):
    def __init__(self):
        ...
        self.mpc = MPC_DynamicBicycle(self.ds, N_horizon=20)
    ...
    def run(self):
        ...
        while rclpy.ok():
            ...
            if (s_sim - prev_s >= self.ds): #or (s_sim>self.mpc.s_ref[i-1])
                ...
                self.v_cmd = self.mpc.trajectory[6, self.idx]
    ...
    def solve_and_apply(self, state_ocp):
        try:
            ...
            self.get_logger().info(
                ...
                f"Yaw {np.rad2deg(yaw):.2f}° | Vel {v:.2f} {v:.2f}m/s | "

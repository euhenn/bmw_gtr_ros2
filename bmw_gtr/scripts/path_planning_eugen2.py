...
class PathPlanning:
    ...
    @staticmethod
    def project_point_on_path(X, Y, clothoids, samples_per_meter=50):
        """
        Project (X, Y) onto a list of clothoids by discretizing each clothoid.

        Returns:
            (x_proj, y_proj), s_global, dist_min, idx_clothoid
        """
        ...
        for i, c in enumerate(clothoids):

            # Number of samples based on clothoid length
            n = max(5, int(c.length * samples_per_meter))
            s_vals = np.linspace(0.0, c.length, n)

            xs = np.array([c.X(s) for s in s_vals])
            ys = np.array([c.Y(s) for s in s_vals])
            ...
            if d[idx] < best_dist:
                ...
                best_x = xs[idx]
                best_y = ys[idx]
                best_s_global = s_accum + s_vals[idx]
                idx_closest = i

            s_accum += c.length

        return (best_x, best_y), best_s_global, best_dist, idx_closest

...
nodes_to_visit = [91,125,141,91,125]
y_ref, s_ref, kappa_ref = track.generating_spatial_reference(ds_ocp, nodes_to_visit)
...
def CreateOcpSolver_SpatialKin() -> AcadosOcp:
    ...
    ocp.cost.W_e = np.diag([1e3,5*1e4]) * ds_ocp
...
def ClosedLoopSimulationSpatial():
    ...
    for i in range(Nsim): 
        ...
        if (s_sim- s_prev >= ds_ocp) or (s_sim >= s_ref[k]): # 
        ...
        acados_sim_solver.set("p", np.array([ s_ref[k] ]))

...
class MPC_KinematicBicycle:
    ...
    def set_reference(self, traj_xyz):
        """
        traj_xyz: shape (3, N_horizon+1) with rows [x_ref, y_ref, psi_ref]
        Sets yref for stages 0..N-1 and terminal yref_e at stage N.
        """
        assert traj_xyz.shape[0] == 3
        assert traj_xyz.shape[1] >= self.N_horizon + 1

        for j in range(self.N_horizon):
            # yref = [x_ref, y_ref, psi_ref + slip_target(=0 here), v_ref(=0), delta_ref(=0)]
            yref = np.concatenate([traj_xyz[:, j], np.array([0.0, 0.0])])
            self.solver.set(j, "yref", yref)

        # terminal: only x,y
        yref_e = traj_xyz[:2, self.N_horizon]
